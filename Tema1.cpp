...
void Tema1::LoadDiamonds() {
	int i;
	for (i = 0; i < 15; i++) {
		if (collisionMemDiamond[i] != 1) { //daca s-a produs anterior coliziune cu sageata, nu mai apare diamantul
	...
	for (i = 0; i < 15; i++) {
		if (diamondsPos[i].y <= window->GetResolution().y) {
			if (collisionMemDiamond[i] != 1) { //daca nu s-a mai produs coliziune pana la acest moment
				...
				if (powf(arrowTop.x - diamondX, 2.0f) + powf(arrowTop.y - diamondY, 2.0f) <
					powf(40.0f, 2.0)) {
...
void Tema1::LoadWindingBallons(float deltaTimeSeconds) {
	int i;
	for (i = 0; i < 20; i++) {
		if (collisionMemWinBall[i] != 1) {
			...
			//memoreaza pozitiile baloanelor
			windingBallons[i] = glm::vec2(modelMatrixBallonY[2][0] + modelMatrixBallonY[0][0],
				modelMatrixBallonY[2][1] + modelMatrixBallonY[1][1]);
		}
	}
	...
	for (i = 20; i < 40; i++) {
		if (collisionMem[i] != 1) {
			modelMatrixBallonY = glm::mat3(1);
			modelMatrixBallonY *= Transform2D::Translate(500, -250 * (i - 20) - 250);
...
{
	//cout << mouseX << " " << window->GetResolution().y - mouseY << ".." <<  modelMatrixArrow[2][0] << ".." << modelMatrixArrow[2][1] << endl;
	mouseClick = true;
	maxSpeed = false;
